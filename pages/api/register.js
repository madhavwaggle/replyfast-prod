...
export default async function handler(req, res) {
  ...
  const { name, email, password, agencyName, agreedToTermsAt } = req.body || {};
  ...
  try {
    const user  = await createUser({ name, email, password, agencyName, agreedToTermsAt: agreedToTermsAt || new Date().toISOString() });
