    ...
    <>
      ...
      {view === 'landing' && (
        <section className="fade-in">
          ...
          / ─── HOW IT WORKS MODAL ──────────────────────────────────────────────────────
// Drop this component anywhere below the existing helper components
// (FAQItem, IntegCard, CredField) near the bottom of index.js
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          border: '1px solid var(--border)', width: '100%', maxWidth: '580px',
      ...
      >
        ...
        <div style={{ padding: '1.5rem 1.5rem 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            <div style={{ fontSize: '18px', fontWeight: '600', color: 'var(--black)', marginBottom: '.25rem' }}>
              How Say HelloLeads works
            </div>
            <div style={{ fontSize: '13px', color: 'var(--muted)' }}>
              Everything that happens automatically — so you can focus on closing.
            </div>
          </div>
          <button
            onClick={onClose}
            style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '8px', width: '30px', height: '30px', cursor: 'pointer', fontSize: '16px', color: 'var(--muted)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}
            aria-label="Close"
          >
            ×
          </button>
          ...
          {tabs.map(t => (
            <button
              ...
              style={{
                ...
                padding: '.5rem .85rem', fontSize: '13px', fontWeight: '500',
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          {tab === 'overview' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                {
                  icon: '👤', color: '#e8efe7', label: 'A buyer reaches out',
                  desc: 'Someone inquires about a property — through Zillow, your website, a text to your number, Facebook, or any source you\'ve connected. Day or night, you\'re covered.',
                },
                {
                  icon: '⚡', color: '#e8efe7', label: 'They get a reply in under 60 seconds',
                  desc: 'A warm, personal message goes out immediately — mentioning the exact property they asked about. It sounds like it came from your assistant, not software. Leads engage because it feels real.',
                },
                {
                  icon: '💬', color: '#e8efe7', label: 'The conversation qualifies the lead',
                  desc: 'The AI naturally follows up to learn their timeline, budget, and whether they\'re pre-approved — without them ever feeling like they\'re filling out a form.',
                },
                {
                  icon: '🔔', color: '#fdf3e7', label: 'You get a notification',
                  desc: 'An alert lands in your email (and by text if you\'ve set that up) with the buyer\'s name, what they\'re looking for, and the full conversation so far.',
                },
                {
                  icon: '🔥', color: '#fdf3e7', label: 'Your dashboard tells you who to call first',
                  desc: 'Every lead gets scored HOT, WARM, or COLD automatically. Hot leads are ready to buy — you\'ll know their budget, timeline, and the best next step to take.',
                },
              ].map((item, i, arr) => (
                <div key={i} style={{ display: 'flex', gap: '14px', paddingBottom: '14px', marginBottom: i < arr.length - 1 ? '0' : 0, borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none', paddingTop: i > 0 ? '14px' : 0 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: '32px' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', flexShrink: 0 }}>
                      {item.icon}
                    </div>
          ...
          {tab === 'sources' && (
            <>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '14px', lineHeight: '1.6' }}>
                Every source below flows into the same dashboard. You set each one up once — after that it runs itself.
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
                {[
                  { emoji: '🏠', title: 'Zillow, Homes.com, Realtor.com', desc: 'You add your unique email address as an extra recipient in each portal\'s notification settings. Buyers inquire — you still get your normal copy, and we handle the instant reply.' },
                  { emoji: '📱', title: 'Text message', desc: 'Buyers text your dedicated number from a yard sign, flyer, or listing. The AI texts back immediately — from that same number, so it feels personal.' },
                  { emoji: '📘', title: 'Facebook & Instagram ads', desc: 'When someone fills out your lead ad form, it routes automatically to your dashboard via Zapier. One-time setup, takes about 10 minutes.' },
                  { emoji: '🌐', title: 'Your website', desc: 'Any contact form on your own site can connect here. Leads arrive exactly the same way as every other source — no extra work after setup.' },
                ].map((s, i) => (
              ...
              <div style={{ background: 'var(--sage-light)', borderRadius: '10px', padding: '12px 14px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '13px', color: 'var(--sage)', lineHeight: '1.6' }}>
                  You don't need all of them. Start with Zillow or your busiest source and add others whenever you're ready. Each one takes 2–5 minutes.
                </div>
              </div>
            </>
          )}
          ...
          {tab === 'scores' && (
            <>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '14px', lineHeight: '1.6' }}>
                After every conversation, your dashboard automatically scores the lead. This tells you exactly where to spend your time.
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '16px' }}>
                {[
                  { bg: '#fde8e8', border: '#f5c6c6', label: '🔥 HOT', labelColor: 'var(--red)', desc: 'Ready to buy soon. Has a budget. Asking specific questions about a property.', action: 'Call within the hour.', actionColor: 'var(--red)' },
                  { bg: 'var(--amber-light)', border: '#f0d9b5', label: '🌤 WARM', labelColor: 'var(--amber)', desc: 'Interested but still exploring. No firm timeline yet. Worth nurturing.', action: 'Follow up weekly.', actionColor: 'var(--amber)' },
                  { bg: '#e8eef8', border: '#c5d0e8', label: '❄️ COLD', labelColor: '#5470a0', desc: 'Just browsing. Early stage, no urgency.', action: 'Stay in touch over time.', actionColor: '#5470a0' },
                ].map((s, i) => (
                  <div key={i} style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: '10px', padding: '12px' }}>
                    ...
                    <div style={{ fontSize: '11px', fontWeight: '600', color: s.actionColor }}>{s.action}</div>
                  </div>
                ))}
              </div>
              <div style={{ background: 'var(--sage-light)', borderRadius: '10px', padding: '12px 14px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                <span style={{ fontSize: '14px' }}>💡</span>
                <div style={{ fontSize: '13px', color: 'var(--sage)', lineHeight: '1.6' }}>
                  Each scored lead also shows the buyer's timeline, budget, whether they're pre-approved, and a suggested next step — so you always know what to say when you call.
                </div>
              </div>
            </>
          )}
          ...
          {tab === 'you' && (
            <>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '16px', lineHeight: '1.6' }}>
                Say HelloLeads handles the first response, the follow-up questions, and the scoring. Here's what's left for you.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {[
                  ...
                  { icon: '🔥', color: '#fdf3e7', label: 'Call your HOT leads', desc: 'When you get a HOT alert, that\'s your cue. The hard work is done — you already know their name, budget, timeline, and whether they\'re pre-approved. Just call.' },
                  { icon: '💬', color: '#e8efe7', label: 'Jump in whenever you want', desc: 'Every conversation is saved in your dashboard. You can read it, continue it yourself, or send a follow-up text — the full history is always there.' },
