...
/**
 * Detect call intent in a lead message.
 */
export function detectCallIntent(text) {
  if (!text) return false;
  return /\bcall me\b|\bgive me a call\b|\bcan you call\b|\bplease call\b|\bcall me at\b|\bphone call\b|\bi prefer.*call\b|\breach me by phone\b|\btalk on the phone\b/i.test(text);
}

/**
 * Urgent agent alert when a lead asks to be called.
 */
export async function notifyAgentCallRequest(lead, agentEmail, agentName, apiKey) {
  // Fire email + SMS in parallel
  await Promise.allSettled([
    notifyCallByEmail(lead, agentEmail, agentName, apiKey),
    notifyCallBySMS(lead),
  ]);
}

async function notifyCallBySMS(lead) {
  ...
  const name  = [lead.fname, lead.lname].filter(Boolean).join(' ') || 'A lead';
  const phone = lead.phone || '—';
  const body  = `📞 CALL NOW: ${name} asked to be called.\nPhone: ${phone}\nProperty: ${lead.property || '—'}\n\nView: ${process.env.NEXTAUTH_URL || 'https://www.sayhelloleads.com'}`.slice(0, 1600);
  try {
    const twilio = (await import('twilio')).default;
    await twilio(accountSid, authToken).messages.create({ to: toNumber, from: fromNumber, body });
  } catch (e) { console.error('Call request SMS error:', e.message); }
}

async function notifyCallByEmail(lead, agentEmail, agentName, apiKey) {
  const key = apiKey || process.env.RESEND_API_KEY;
  if (!agentEmail || !key) return;
  const name     = [lead.fname, lead.lname].filter(Boolean).join(' ') || 'This lead';
  const phone    = lead.phone || '—';
  const property = lead.property || '—';
  const lastMsg  = lead.messages?.filter(m => m.role === 'lead').slice(-1)[0]?.text || '';
  const html = `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:520px;margin:0 auto;color:#0a0a0a;">
      <div style="background:#c0392b;padding:24px 32px;border-radius:12px 12px 0 0;">
        <div style="font-size:20px;font-weight:700;color:#fff;">📞 Lead wants a call — call now</div>
        <div style="color:rgba(255,255,255,.8);font-size:14px;margin-top:4px;">High intent · act within the hour</div>
      </div>
      <div style="background:#fafaf8;border:1px solid #e0ddd8;border-top:none;padding:28px 32px;border-radius:0 0 12px 12px;">
        <table style="width:100%;font-size:14px;border-collapse:collapse;">
          <tr><td style="padding:7px 0;color:#6b6b6b;width:130px;">Name</td><td style="padding:7px 0;font-weight:600;">${name}</td></tr>
          <tr><td style="padding:7px 0;color:#6b6b6b;">Phone</td><td style="padding:7px 0;font-weight:700;font-size:16px;"><a href="tel:${phone}" style="color:#c0392b;">${phone}</a></td></tr>
          <tr><td style="padding:7px 0;color:#6b6b6b;">Property</td><td style="padding:7px 0;">${property}</td></tr>
          <tr><td style="padding:7px 0;color:#6b6b6b;">Score</td><td style="padding:7px 0;">${lead.score === 'HOT' ? '🔥 HOT' : lead.score === 'WARM' ? '🌤️ WARM' : '❄️ COLD'}</td></tr>
        </table>
        ${lastMsg ? `<div style="margin-top:16px;padding:12px 16px;background:#fdf0ee;border-radius:8px;font-size:13px;color:#c0392b;border-left:3px solid #c0392b;">
          <strong>What they said:</strong><br>"${lastMsg}"
        </div>` : ''}
        <div style="margin-top:16px;padding:12px 16px;background:#fff8e6;border-radius:8px;font-size:13px;color:#8a6800;">
          💡 Call within the hour. Leads who ask to be called expect a fast response — every hour reduces conversion by ~20%.
        </div>
      </div>
    </div>`;
  try {
    ...
    await resend.emails.send({
      ...
      to:      agentEmail,
      subject: `📞 ${name} wants a call — ${phone}`,
      html,
    });
  } catch (e) { console.error('Call request email error:', e); }
}
...
/**
 * Lightweight email-only notification when a lead replies but score stays WARM or COLD.
 * No SMS — keeps agent's phone quiet for non-urgent replies.
 * Agent still sees the full conversation on the dashboard.
 */
export async function notifyAgentLeadReply(lead, agentEmail, agentName, apiKey) {
  const key = apiKey || process.env.RESEND_API_KEY;
  if (!agentEmail || !key) return;
  ...
  const scoreColor = lead.score === 'HOT' ? '#c0392b' : lead.score === 'WARM' ? '#c9873a' : '#5470a0';
  const dashUrl  = process.env.NEXTAUTH_URL || 'https://www.sayhelloleads.com';

  const html = `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:520px;margin:0 auto;color:#0a0a0a;">
      <div style="background:#4a6741;padding:20px 32px;border-radius:12px 12px 0 0;">
        <div style="font-size:18px;font-weight:700;color:#fff;">💬 ${name} replied</div>
        <div style="color:rgba(255,255,255,.8);font-size:13px;margin-top:3px;">${property}</div>
      </div>
      <div style="background:#fafaf8;border:1px solid #e0ddd8;border-top:none;padding:24px 32px;border-radius:0 0 12px 12px;">
        ${lastMsg ? `
        <div style="background:#f0f0ec;border-radius:10px;padding:14px 16px;margin-bottom:20px;font-size:14px;line-height:1.6;color:#0a0a0a;border-left:3px solid #e0ddd8;">
          "${lastMsg}"
        </div>` : ''}
        <table style="width:100%;font-size:13px;border-collapse:collapse;margin-bottom:20px;">
          <tr><td style="padding:5px 0;color:#6b6b6b;width:100px;">Lead</td><td style="padding:5px 0;font-weight:600;">${name}</td></tr>
          <tr><td style="padding:5px 0;color:#6b6b6b;">Property</td><td style="padding:5px 0;">${property}</td></tr>
          <tr><td style="padding:5px 0;color:#6b6b6b;">Score</td><td style="padding:5px 0;font-weight:600;color:${scoreColor};">${scoreEmoji} ${lead.score || 'WARM'}</td></tr>
          <tr><td style="padding:5px 0;color:#6b6b6b;">Source</td><td style="padding:5px 0;">${lead.source || '—'}</td></tr>
        </table>
        <a href="${dashUrl}/?view=dashboard" style="display:inline-block;background:#4a6741;color:#fff;text-decoration:none;padding:10px 24px;border-radius:8px;font-size:13px;font-weight:600;">
          View conversation →
        </a>
        <div style="margin-top:16px;font-size:12px;color:#999;">
          The AI has already replied. Check the dashboard to review the conversation or jump in yourself.
        </div>
      </div>
    </div>`;
  ...
  try {
    ...
    await resend.emails.send({
      from:    `Say HelloLeads <${fromAddress}>`,
      to:      agentEmail,
      subject: `💬 ${name} replied — ${scoreEmoji} ${lead.score || 'WARM'}`,
      html,
    });
  } catch (e) { console.error('Lead reply notification error:', e); }
}
