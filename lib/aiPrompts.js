...
export function buildConversationPrompt({ agentName, lead, conversationHistory }) {
  ...
  // ── Parse what the lead has already told us directly from messages ──────
  // Don't rely solely on lead.signals — scoring may not have run yet or
  // may have returned null for fields the lead clearly answered in plain text.
  const leadMessages = (lead.messages || []).filter(m => m.role === 'lead').map(m => m.text.toLowerCase());
  const aiMessages   = (lead.messages || []).filter(m => m.role === 'ai').map(m => m.text.toLowerCase());
  const allLeadText  = leadMessages.join(' ');
  const allAiText    = aiMessages.join(' ');

  // What has the lead mentioned?
  const mentionedTimeline = /\b(\d+\s*day|\d+\s*week|\d+\s*month|asap|immediately|right away|soon|within|next month|this month|this year|couple of month)/i.test(allLeadText);
  ...
  // What has the AI already asked?
  const askedTimeline    = /timeline|when.*moving|how soon|when.*looking|looking to move/i.test(allAiText);
  ...
  // Build the explicit known-facts block
  const established = [];
  if (mentionedTimeline || (lead.signals?.timeline))
    established.push(`timeline: lead already answered this — do NOT ask again`);
  if (mentionedBudget || lead.signals?.budget)
    established.push(`budget: lead already answered this — do NOT ask again`);
  if (mentionedPreApproval || lead.signals?.preApproved != null)
    established.push(`pre-approval: lead already answered this — do NOT ask again`);
  if (mentionedPhone || askedPhone)
    established.push(`phone/callback: already collected or asked — do NOT ask again`);

  // What's still unknown — build the next question guidance
  const nextQuestions = [];
  if (!mentionedTimeline && !lead.signals?.timeline && !askedTimeline)
    nextQuestions.push('timeline (when are they looking to move?)');
  if (!mentionedBudget && !lead.signals?.budget && !askedBudget)
    nextQuestions.push('budget (do they have a price range in mind?)');
  if (!mentionedPreApproval && lead.signals?.preApproved == null && !askedPreApproval && leadMessages.length >= 2)
    nextQuestions.push('pre-approval (have they been pre-approved?)');
  if (established.length >= 2 && nextQuestions.length === 0)
    nextQuestions.push('schedule a showing or suggest a quick call');

  const knownBlock = established.length > 0
    ? `\nALREADY ESTABLISHED — DO NOT ASK ABOUT THESE AGAIN:\n${established.map(f => `- ${f}`).join('\n')}`
    : '';

  const nextBlock = nextQuestions.length > 0
    ? `\nNEXT THING TO UNCOVER (pick ONE only):\n- ${nextQuestions[0]}`
    : '\nNEXT STEP: Move toward scheduling a showing or a call.';
...
The full conversation is in your context. READ IT before replying.
If the lead has already answered something — ACKNOWLEDGE IT and move on.
If you already asked something — DO NOT ask it again under any circumstances.
Asking the same question twice makes you look like a bot. It is the worst thing you can do.

Example of what NOT to do:
- Lead says "30 days" → You ask "what's your timeline?" again → WRONG
- Lead gives phone number → You ask for callback number → WRONG

Example of what TO do:
- Lead says "30 days" → You say "Perfect, 30 days works — do you have a budget in mind?" → RIGHT

ANSWERING RULE:
Always answer their question directly FIRST before asking anything.
Never ignore or dodge a question.

INTENT ADAPTATION:
- Low intent → keep it light, soft qualification
- Medium intent → guide toward next step
- High intent (wants to see it, asks about offers) → move quickly to scheduling

OBJECTION HANDLING:
- "Just browsing" → low-pressure, no push
- Not pre-approved → "No worries — happy to connect you with a great lender."
- Working with another agent → politely disengage

SOFT CLOSE:
- Before suggesting a call, get one confirming step first
- "Got it — want me to have ${agentName} give you a quick call?"

NEVER:
- Ask about anything listed in ALREADY ESTABLISHED above
- Repeat a question from earlier in this conversation
